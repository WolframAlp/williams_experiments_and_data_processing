...
class ImageSeries:
    ...
    def __init__(self, main=None, others=None, center=None):
        ...
        if others is not None:
            ...
            # Gets unique instances and separates the image type
            # self.others_names = list(set(undersplit))
            # self.others_names = [name.split('.')[0] for name in self.others_names]
            ...
            for n,name in enumerate(self.others_names):
                ...
                # print(f"Name: {n}/{self.num_settings}")
                
#                 instances = 0
                
#                 for content in others_content:
#                     if name in content:
#                         if instances == 0:
#                             image = np.array(Image.open(content), dtype=float)
#                         else:
#                             image += np.array(Image.open(content), dtype=float)
#                         instances += 1
#                 self.others_data[name] = image/instances
#                 instances = 0
    ...
    def subtract_backgrounds(self, backgrounds):
        '''Subtracts the provided background from all the other images'''
        for im, background in zip(self.others.items(), backgrounds.items()):
